...
/* Self-defined strcpy(). */
void _strcpy(char* tgt, char* src)
{
	while(*tgt++ = *src++);
}

/* Self-defined strcat(). */
void _strcat(char* tgt, char* src)
{
	while(*tgt++);
	while(*tgt++ = *src++);
}
	...
	while (remainPatientCount || remainNurseCount) {
		...
		if(nurseWaitWrnCount > 0) {
			...
			if(indexOfSheet > numOfNursesPassed)
			{
				...
				copyExamSheet(wrnExamSheet[examSheetIndex], examSheetArray[indexOfPatient++]);
...
void Patient() 
{
	...
	copyExamSheet(wrnExamSheet[examSheetIndex], myExamSheet);
...
int main(void)
{
	...
	numberOfChildren = numberOfParents = 3;
